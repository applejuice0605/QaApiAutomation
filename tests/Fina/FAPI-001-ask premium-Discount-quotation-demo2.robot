...
Library    ../../resources/util/testAlert.py
...
遍历执行测试用例
#    ${data}    Read Excel File    ${BODY_FILE_PATH}    sheet_name=Sheet1
#    ${data}=    Set Variable    {"apiAutomationTicket":"testcase1","scenario":"testcase1","testSteps":[{"stepName":"Send Quote","msgType":"text","content":"quote","alertSteps":[{"reply_id":"quotation_form_new"}]},{"stepName":"Input car info","msgType":"text","content":"Toyota Innova 2022 B","alertSteps":[{"reply_id":"quotationCollectedFormMessage"}]},{"stepName":"Sumbit quickquote form","msgType":"flow","content":"{\\\"brandCode\\\":\\\"TOYOTA\\\",\\\"vehicleManufactureYear\\\":\\\"2022\\\",\\\"brandModelCode\\\":\\\"ALL NEW KIJANG INNOVA\\\",\\\"brandSeriesCode\\\":\\\"2.0 G A\\/T BENSIN\\\",\\\"vehicleType\\\":\\\"MINIBUS\\\",\\\"vehicleTypeCode\\\":\\\"MINIBUS\\\",\\\"carPriceShow\\\":\\\"376,200,000\\\",\\\"coveragePeriod\\\":\\\"1\\\",\\\"plateCode\\\":\\\"B\\\",\\\"vehicleUsage\\\":\\\"2\\\",\\\"flow_token\\\":\\\"QUICK_QUOTE:8619830441461:25550e154d404cae9da8ab1abbbf7e82\\\"}","alertSteps":[{"reply_id":"newCommissionCompareMessage"}]},{"stepName":"upload document","msgType":"document","content":"22121212888","alertSteps":[{"reply_id":"newCommissionCompareMessage"}]},{"stepName":"click button","msgType":"button","content":"{\\\"id\\\": \\\"Renewa\\\",\\\"title\\\":\\\"Ya\\\"}","alertSteps":[{"reply_id":"newCommissionCompareMessage"}]}]}   {"apiAutomationTicket":"testcase1","scenario":"testcase1","testSteps":[{"stepName":"Send Quote","msgType":"text","content":"quote","alertSteps":[{"reply_id":"quotation_form_new"}]},{"stepName":"Input car info","msgType":"text","content":"Toyota Innova 2022 B","alertSteps":[{"reply_id":"quotationCollectedFormMessage"}]}]}
    ${data}   Create List     {"apiAutomationTicket":"testcase1","scenario":"testcase1","testSteps":[{"stepName":"Send Quote","msgType":"text","content":"quote","alertSteps":[{"reply_id":"comparisonStartMessage"}]},{"stepName":"Input car info","msgType":"text","content":"Toyota Innova 2022 B","alertSteps":[{"reply_id":"quotationCollectedFormMessage"}]},{"stepName":"Sumbit quickquote form","msgType":"flow","content":"{\\\"brandCode\\\":\\\"TOYOTA\\\",\\\"vehicleManufactureYear\\\":\\\"2022\\\",\\\"brandModelCode\\\":\\\"ALL NEW KIJANG INNOVA\\\",\\\"brandSeriesCode\\\":\\\"2.0 G A\\/T BENSIN\\\",\\\"vehicleType\\\":\\\"MINIBUS\\\",\\\"vehicleTypeCode\\\":\\\"MINIBUS\\\",\\\"carPriceShow\\\":\\\"376,200,000\\\",\\\"coveragePeriod\\\":\\\"1\\\",\\\"plateCode\\\":\\\"B\\\",\\\"vehicleUsage\\\":\\\"2\\\",\\\"flow_token\\\":\\\"QUICK_QUOTE:8619830441461:25550e154d404cae9da8ab1abbbf7e82\\\"}","alertSteps":[{"reply_id":"newCommissionCompareMessage"}]},{"stepName":"click button","msgType":"button","content":"{\\\"id\\\": \\\"addDiscount\\\",\\\"title\\\":\\\"Tambah Diskon\\\"}","alertSteps":[{"reply_id":"price_comparision_pdf"},{"reply":"price_comparision_id_0508"}]},{"stepName":"Select Plan","msgType":"button","content":"{\\\"id\\\": \\\"MP_00003\\\",\\\"title\\\":\\\"Plan A\\\"}","alertSteps":[{"reply_id":"policy_type_option"}]},{"stepName":"Select policy type = New Order, click button Tidak","msgType":"button","content":"{\\\"id\\\": \\\"New Order\\\",\\\"title\\\":\\\"Tidak\\\"}","alertSteps":[{"reply_id":"newOrderFlowMessage"}]},{"stepName":"Select car condition = New Car, click button Mobil Bekas","msgType":"button","content":"{\\\"id\\\": \\\"Old Car\\\",\\\"title\\\":\\\"Mobil Bekas\\\"}","alertSteps":[{"reply_id":"collect_old_car_information"}]},{"stepName":"upload document","msgType":"document","content":"699509946392104","alertSteps":[{"reply_id":"ocrLoadingMessage"},{"reply_id":"quotation_form_new"}]}]}
    Log    ${data}

    FOR    ${caseItem}    IN    @{data}
        Log    ${caseItem}
        ${caseItem}    Convert String To JSON    ${caseItem}
#        Log    ${caseItem.__class__}
        ${scenario}   Get From Dictionary    ${caseItem}    scenario
        Log    ${caseItem}[scenario]
        Log    ${caseItem}[testSteps]
        FOR    ${step}    IN    @{caseItem}[testSteps]
            Log    ${step}[stepName]
            ...
            Log    ${step}[msgType]
            Log    ${step}[content]
            ${msg_type}  Set Variable    ${step}[msgType]
            ${content}  Set Variable    ${step}[content]
            ${msg_body}=    Get Msg Body   type=${msg_type}     content=${content}
            Log    ${msg_body}
#            ${request_body}=    Set Variable    {"object":"whatsapp_business_account","entry":[{"id":"1851481102271082","changes":[{"value":{"messaging_product":"whatsapp","metadata":{"display_phone_number":"6285283239812","phone_number_id":"539655932572544"},"contacts":[{"profile":{"name":"nora 2号"},"wa_id":"8619830441461"}],"messages":[${msg_body}]},"field":"messages"}]}]}
#            Log    ${request_body}
            # -》发送请求
            ...
            ${trace_id}=    Set Variable    ${resonse.text}
...
#                IF    ${response_data} == [] or ${response_data} == ${None}
            ...
            #-》》 断言
            Log    ${step}[alertSteps]
            FOR    ${alertItem}    IN    @{step}[alertSteps]
                Log    ${alertItem}
                ${response_item}=    Set Variable   ${response_data}[index]

                ${index}=    Get Index From List    @{step}[alertSteps]    ${alertItem}
                ${target_keys}=    Get Dictionary Keys    ${alertItem}
                ${target_key}=    ${target_keys}[0]
                ${target_value}=    Get From Dictionary    ${alertItem}    ${target_key}

                ${response_value}=    Get From Dictionary    ${response_item}    ${target_key}
                Should Be Equal    ${target_value}    ${response_value}
            END
