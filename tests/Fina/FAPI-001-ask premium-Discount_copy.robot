...
ask premium
    ...
    Given Login in Fina
    When Send "Quote"
    Then Check Msg ID Match     whatsapp_id=${whatsapp_id}  target_key="reply_id"   expeceted_value="quotation_form_new"
    Then Input car info
    Then Check Msg ID Match     whatsapp_id=${whatsapp_id}  target_key="reply_id"   expeceted_value="quotationCollectedFormMessage"
    Then Sumbit QuickQuote Form
    Then Check Msg ID Match     whatsapp_id=${whatsapp_id}  target_key="reply_id"   expeceted_value="newCommissionCompareMessage"
    Then Click button "Tambah Diskon"
    Then Check Msg ID Match     whatsapp_id=${whatsapp_id}  target_key="reply_id"   expeceted_value="price_comparision_pdf"
    Then Check Msg ID Match     whatsapp_id=${whatsapp_id}  target_key="reply"   expeceted_value="price_comparision_id_0508"
