...
*** Variables ***
${ORDER_MSG_BODY_FILE_PATH}     Travel_PlaceOrderData.json
...
Create Pay Now Order
    ...
    # 调用询价业务
    travel_order.I send the quotation request to savebinderrfq API    ${ORDER_MSG_DATA}    ${token}
    assertUtil.The response should contain the value quoteNo and rfqNo    ${jsonResult}
    # 调用下单业务
    travel_order.I send the place order request to createrfqorder API     ${ORDER_MSG_DATA}    ${token}    ${rfqNo}    ${quoteNo}    ${isAdvancePremium}
