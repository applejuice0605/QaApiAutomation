...
${amount}=   10973000
*** Test Cases ***
Upload payment proof with underpayment success
...
Send upload Post Request
    ${actualAmount}=  Evaluate    ${amount}-10000
    ${response}=  Send UploadProof Post Request  ${paymentBillNo}   ${fuseToken}  ${actualAmount}
    ${get_data}=  Get From Dictionary    ${response.json()}  header
    ${message}=  Get From Dictionary    ${get_data}  message
    Should Be Equal As Strings    ${message}    The Actual Payment Amount is less than the Receivable Amount.
#    Set Test Variable    ${actualAmount}  ${actualAmount}
...
Go Task PaymentBillingFlow Approval
    Send PaymentBillingFlow Approval Request   ${paymentBillNo}   ${paymentSlipNo}  ${referenceNo}  ${id}  ${fuseToken}   ${actualAmount}
