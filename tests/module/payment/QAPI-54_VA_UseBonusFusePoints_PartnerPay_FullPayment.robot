...
I choose Partner Pay & Full payment & VA payment method and send request to /slip/process API
    ${response}    slip_process.Send Request And Get Response Data    token=${token}    orderId=${orderId}    securityCode=${securityCode}  paymentScheme=3    payerType=2   bonusDeduction=100     pointsDeduction=100000
    Set Test Variable    ${jsonResult}    ${response.json()}
    Log    ${jsonResult}
