...
*** Test Cases ***
Use Full Bonus PartnerPay FullPayment Car Order
    [Tags]    uat   wallet
    ...
    Then Use Full Bonus and send the request to /slip/process API     token=${token}     orderId=${orderId}     securityCode=${securityCode}     PaymentScheme=${PaymentScheme}
    Then The status code should be 200    ${jsonResult}[code]
