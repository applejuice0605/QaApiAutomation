...
*** Test Cases ***
Use Full Bonus And FusePoints PartnerPay FullPayment Car Order
    [Tags]    uat   wallet
    ...
    Then Use Full Bonus And FusePoints and send the request to /slip/process API     token=${token}     orderId=${orderId}     securityCode=${securityCode}     PaymentScheme=${PaymentScheme}
    Then The status code should be 200    ${jsonResult}[code]
    And the response should contain lessAmount and lessAmount should be 0      ${jsonResult}
