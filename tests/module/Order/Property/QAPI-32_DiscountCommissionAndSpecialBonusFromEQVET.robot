...
${payerType}    2
${paymentScheme}    1
${discountFormCommission_EQVET}     2077
${discountFromPartnerSpecialBonusAmount_EQVET}      120
${paymentMethod}    VA
...
Order Property and EQVET using discountCommission and specialBonus from EQVET PayLater PartnerPay Fullpayment
    ...
    Then I send the place order request to createrfqorder API    ${AP_POSITIVE_DATA}     ${token}    ${rfqNo}    ${quoteNo}  ${isAdvancePremium}    discountFormCommission_EQVET=${discountFormCommission_EQVET}    discountFromPartnerSpecialBonusAmount_EQVET=${discountFromPartnerSpecialBonusAmount_EQVET}
    Then The status code should be 200    ${jsonResult}[code]
