...
${paymentScheme}    1
${payerType}    2
${discountFormCommission_property}      1634
...
Property EQVET PayLater With Discount Commission And Special Bonus From Property
    ...
    Then I send the place order request to createrfqorder API    ${AP_POSITIVE_DATA}     ${token}    ${rfqNo}    ${quoteNo}  ${isAdvancePremium}    discountFormCommission_property=${discountFormCommission_property}    discountFromPartnerSpecialBonusAmount_property=${discountFromPartnerSpecialBonusAmount_property}
    Then The status code should be 200    ${jsonResult}[code]
