...
Net Payment With EQVET Coupon
    ...
    Then I choose partner pay & Full payment & a payment method amd send request to /slip/process API
    Then The status code should be 200    ${jsonResult}[code]
...
the response should contain the available coupon list
    ${couponDTO}=    utilCommon.Get CouponId by ProductCode    ${jsonResult}[data]    ${product_code_use_coupon}
...
I choose partner pay & Full payment & a payment method amd send request to /slip/process API
