...
Car DoubleOrder ClassisNumber already registered
    ...
    Then I send the place order request to createrfqorder API using register classisNumber    ${AP_POSITIVE_DATA}     ${token}    ${rfqNo}    ${quoteNo}  ${isAdvancePremium}    vehicleVinNo=${vehicleVinNo}
...
When I have a whitelist account and have logined and have a active order
    ...
    Set Test Variable    ${vehicleVinNo}
    # 调用询价业务
    ...
    # 调用下单业务
    vehicle_order.I send the place order request to createrfqorder API     ${AP_POSITIVE_DATA}    ${token}    ${rfqNo}    ${quoteNo}    ${isAdvancePremium}     vehicleVinNo=${vehicleVinNo}
