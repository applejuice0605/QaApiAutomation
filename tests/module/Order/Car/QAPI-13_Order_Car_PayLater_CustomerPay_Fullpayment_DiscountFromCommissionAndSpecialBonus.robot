...
*** Test Cases ***
Car PayLater CustomerPay Fullpayment using discount from Commission and SpecialBonus
    [Tags]    uat   order-car
    ...
    Then I send the place order request to createrfqorder API    ${AP_POSITIVE_DATA}     ${token}    ${rfqNo}    ${quoteNo}  ${isAdvancePremium}    discountFormCommission=${discountFormCommission}    discountFromPartnerSpecialBonusAmount=${discountFromPartnerSpecialBonusAmount}
    Then The status code should be 200    ${jsonResult}[code]
