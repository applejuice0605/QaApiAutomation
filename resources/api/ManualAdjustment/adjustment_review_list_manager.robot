...
Send Request And Get Response Data
    ...
    [Arguments]    ${bossToken}     ${transactionId}     ${action}
    # 1. 准备请求数据：请求路径、请求头、请求数据
    ${base_url}=   Set Variable     https://boss-uat.fuse.co.id
    ${path}=   Set Variable     /api/adjustment/review/list/manager
    ...
    ${payload}=    Set Variable     {"pageNo":1,"pageSize":20,"transactionId":"${transactionId}","endTask":0,"endProcess":0,"existsAssignee":true}
    # 2. 发送请求
